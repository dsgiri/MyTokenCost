...
export function Header() {
  ...
  return (
    <header className="border-b border-slate-200/80 dark:border-slate-800/80 bg-white/85 dark:bg-slate-950/85 backdrop-blur-md sticky top-0 z-40 transition-colors duration-300">
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-200 dark:border-slate-800/85 bg-white dark:bg-slate-950 px-4 py-6 space-y-6 animate-fade-in transition-colors duration-300">
          <nav className="flex flex-col space-y-4 px-2">
            <Link 
              href="/blueprints" 
              onClick={() => setIsMobileMenuOpen(false)}
              className="text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition uppercase tracking-wider font-semibold"
            >
              Blueprints
            </Link>
            <Link 
